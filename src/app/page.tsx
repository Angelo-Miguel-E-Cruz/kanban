...
export default function Home() {
  ...
  const addNewColumn = () => {
    ...
    const headerColor = colors.find((column) => column.name === selectedColor)
  ...
  return (
    <main>
            ...
            {columns.length === 0 ? (
              ...
              Object.keys(columns).map((columnId, index) => (
                <div
                  ...
                  <div
                    className={`p-4 text-white font-bold text-xl rounded-t-md w-full bg-gradient-to-r ${columns[index].from} ${columns[index].to}`}>
                    {columns[index].name}
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)}>
        <div className="flex items-center justify-center flex-col gap-4 w-full max-w-6xl">
          <h1 className="text-3xl font-bold mb-3 text-yellow-500">Add Column</h1>
